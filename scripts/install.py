...
def _run_cuda_diagnostics() -> None:
    ...
    for base in [
        os.path.join(_prog, "NVIDIA GPU Computing Toolkit", "CUDA"),
        os.path.join(_prog, "NVIDIA", "CUDA"),
    ]:
...
def cuda_setup(auto: bool = False) -> None:
    ...
    if choice == "1":
        ...
        if not cuda_path:
            ...
            for candidate in [
                os.path.join(_cuda_base, "v12.4"),
                os.path.join(_cuda_base, "v12.2"),
                os.path.join(_cuda_base, "v12.1"),
            ]:
