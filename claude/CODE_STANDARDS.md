...
- **Config access:** Use `src/utils/config.py` helpers (`get_rules()`, `get_heartbeat_config()`, etc.). Never read yaml files directly in business logic.
...
- **No cross-module private state access** — Never import `_private_variables` from another module. Instead, add a public function/method to expose the needed behavior. Example: `discord_bot.kick_heartbeat()` instead of importing `discord_bot._heartbeat`. Established session 75, updated session 89.
