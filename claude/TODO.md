...
- [x] **Fix Claude escalation not firing on brainstorm retries** — (Added 2026-02-21, session 63. Fixed 2026-02-21, session 63.) `escalate_for_task()` was called without the required `"claude-sonnet-4.6"` alias argument, causing a `TypeError` silently swallowed by `except Exception: pass`. Also the context manager reference was lost (calling `escalate_for_task().__enter__()` without saving the object). Fixed: store context manager, pass alias, clean `__exit__`. Also increased brainstorm `max_tokens` from 600 → 4096 (was truncating Claude responses), and added raw response logging for debugging. Touches: `idea_generator.py`.
- [x] **Cold-start filter bypass** — (Added 2026-02-21, session 63. Fixed 2026-02-21, session 63.) When there are no active projects and no interests (fresh reset), the relevance and purpose-driven filters were rejecting every idea since there was nothing to be "relevant" to. Now detects cold-start condition and skips those filters, letting ideas through so the user can seed context by choosing what interests them. Duplicate, staleness, and memory-based filters still apply. Touches: `idea_generator.py`.
- [x] **Richer suggestion descriptions** — (Added 2026-02-21, session 63. Fixed 2026-02-21, session 63.) Suggestions sent to the user now include the `reasoning` field from brainstorm output so the user understands what each idea does and why. Formatter prompt updated to explain ideas rather than just listing titles. Description limit 150→250 chars, reasoning up to 250 chars. Discord 2000-char message limit respected. Touches: `notification_formatter.py`.
- [x] **Router misclassifying questions about suggestions as acceptance** — (Added 2026-02-21, session 63. Fixed 2026-02-21, session 63.) "Tell me more about #2" was classified as `suggestion_pick` instead of a question. Added explicit guidance to router prompt distinguishing questions about suggestions (classify as `new_request`, tier `easy`, answer the question) from actual picks (clear acceptance language). Touches: `conversational_router.py`.
- [x] **Shutdown cascade prevention** — (Added 2026-02-21, session 63. Fixed 2026-02-21, session 63.) During Ctrl+C shutdown, cancelled tasks triggered QA rejection → Claude escalation → retry → immediate cancel → next task start → immediate cancel. Three fixes: (1) `autonomous_executor.py` checks for shutdown before QA retry escalation. (2) `task_orchestrator.py` checks `stop_flag` before submitting newly unblocked tasks. (3) Prevents wasted API calls and confusing log output during shutdown. Touches: `autonomous_executor.py`, `task_orchestrator.py`.
- [x] **Restrict file creation to workspace/ directory** — (Added 2026-02-21, session 63. Fixed 2026-02-21, session 63.) Added `_validate_write_path()` to `tool_registry.py` that restricts `create_file` and `append_file` to `workspace/` and `data/` directories only. Prevents Archi from accidentally creating orphan files in the codebase, config, or other sensitive areas. `write_source`/`edit_file` retain their existing approval flow for `src/` modifications. `read_file` and `list_files` remain unrestricted within the project root. Touches: `tool_registry.py`.
- [x] **QueueHandler logging fix** — (Added 2026-02-21, session 63. Fixed 2026-02-21, session 63.) Windows console write blocking (`StreamHandler.emit()`) froze worker threads mid-task because `logger.info()` acquired the handler lock on the same thread. Fix: `archi_service.py` replaces the root logger's console handler with a `QueueHandler` + `QueueListener` pair. `logger.info()` now enqueues instantly (non-blocking), a dedicated daemon thread writes to console/file. Worker threads continue even if console I/O blocks. Touches: `archi_service.py`.
- [x] **Speed up dream cycle testing** — (Added 2026-02-21, session 64. Fixed 2026-02-21, session 64.) Reduced idle_threshold 300→60s, check_interval 30→10s in `heartbeat.yaml`. Reduced suggest_cooldown_base 600→120s in `dream_cycle.py`. These are development values — bump to 900s idle threshold for production. Touches: `heartbeat.yaml`, `dream_cycle.py`.
- [x] **Fix dream cycle suggesting work during active execution** — (Added 2026-02-21, session 64. Fixed 2026-02-21, session 64.) Dream cycle fired new suggestions while goals were actively running (workers busy on API calls). Fix: `_should_run_cycle()` now checks `goal_worker_pool.is_working()` and skips if workers are busy. Touches: `dream_cycle.py`.
- [x] **Fix CancelledError traceback in goal_worker_pool** — (Added 2026-02-21, session 64. Fixed 2026-02-21, session 64.) `_on_goal_done` callback called `future.exception()` on a cancelled future, raising `CancelledError`. Fix: check `future.cancelled()` first and return early. Touches: `goal_worker_pool.py`.
- [x] **Fix shutdown zombie processes (root cause: httpx transport stays open)** — (Added 2026-02-21, session 64. Fixed 2026-02-21, session 64.) Root cause of zombie processes after Ctrl+C: ThreadPoolExecutor worker threads blocked on httpx API calls (20-60s) can't be interrupted, and Python's atexit handler joins all executor threads before exiting. Fix: `router.close()` closes all OpenAI client httpx transports on shutdown, immediately failing in-flight requests and unblocking worker threads. `OpenRouterClient._closed` flag short-circuits the retry loop. `GoalWorkerPool.shutdown(wait=True)` completes quickly since threads are unblocked. No `os._exit()` needed — Python exits normally. Touches: `openrouter_client.py`, `router.py`, `goal_worker_pool.py`, `archi_service.py`.
- [x] **Ask-first proactive initiative ordering** — (Added 2026-02-21, session 64. Fixed 2026-02-21, session 64.) Archi was trying proactive initiative BEFORE asking the user for work. Now: `_ask_user_for_work()` runs first. Only after `_unanswered_suggest_count > 0` (user ignored previous suggestions) does Archi try `_try_proactive_initiative()`. Touches: `dream_cycle.py`.
- [x] **Notification grounding constraint** — (Added 2026-02-21, session 64. Fixed 2026-02-21, session 64.) After reset, Grok claimed ideas were "based on our chats" with no chat history. Fix: added grounding constraint to `_PERSONA` in `notification_formatter.py`: "Only reference information actually provided below. Never claim ideas came from past conversations, user interests, or context that isn't explicitly given to you." Touches: `notification_formatter.py`.
- [ ] **Startup on boot (visible terminal)** — Get Archi auto-starting on laptop reboot again. Must launch in a visible terminal window, not as a background service — if Jesse logs in he needs to see it running.
