...
Last updated: 2026-03-06 (session 214)
...
- [ ] **"test" notification generation continues AFTER restart** — (Added session 210. Investigated sessions 211-214.) **Key finding (session 214):** The 13:53 restart did NOT include session 213's defense-in-depth fix — session 213 was committed at 15:24, 1.5 hours AFTER the restart. The running process has session 211 code (garbage guard in `send_notification` since session 189, but NOT the `_log_outbound` guard from session 213). Yet "test" still bypasses `send_notification`'s guard somehow — all code paths verified, `_is_garbage_notification("test")` returns True at multiple levels. 28 "test" entries on Mar 6 (clusters at 07:02-07:59, 10:27-10:41, 14:28-14:41). All have `cost_usd: 0` and source `dream_cycle_outbound`. **Needs another restart** to deploy session 213's `_log_outbound` guard. If "test" still appears after that, a truly unknown code path exists. Jesse should also verify only one Python process is running (`tasklist | findstr python`). **Files:** `src/interfaces/discord_bot.py`.
...
- [ ] **Scheduled task payload blocked by garbage guard** — (Added 2026-03-06, session 213.) "Drink water" payload (11 chars, 2 words) was caught by `_is_garbage_notification` as garbage (`len < 15 and words <= 2`). Fixed: `_fire_scheduled_task` now wraps short payloads (<20 chars) with "Reminder: " prefix. **Not yet deployed — needs restart.** **File:** `src/core/heartbeat.py`.

- [ ] **Scheduled tasks `times_fired: 0` — verify after next restart** — (Added 2026-03-06, session 213. Investigated session 214.) Session 214 confirmed: `times_fired` still 0 for both tasks. Code analysis shows `advance_task` → `compute_next_run` handles missing croniter gracefully (falls back to +1h). `check_due_tasks` uses simple string comparison, no croniter needed. Most likely cause: FUSE mount showing stale snapshot from before fire times (15:00 and 16:20), or the running process (pre-session-213 code) sends "Drink water" which is garbage-guarded but `advance_task` still runs. **Needs restart + verification.** **Files:** `src/core/scheduler.py`, `src/core/heartbeat.py`.
...
(Sessions 1–203 archived to `claude/archive/COMPLETED_WORK_SESSIONS_1_96.md` and earlier TODO.md entries.)
