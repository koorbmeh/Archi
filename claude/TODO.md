...
Last updated: 2026-03-06 (session 213)
...
- [ ] **"test" notification generation continues AFTER restart** — (Added session 210. Investigated sessions 211-213.) Session 213: confirmed "test" still appears after 13:53 restart with new code. 5 new "test" entries from 14:28-14:41 (post-restart). The garbage guard code is correct and tested (`test_is_garbage_notification` confirms "test" returns True). The code path: `send_notification("test")` → `_is_garbage_notification("test")` → should return True → should block. Yet "test" reaches `_log_outbound` and Discord. **Defense-in-depth fix applied:** added garbage guard inside `_log_outbound` itself, upgraded guard log level to WARNING. **Root cause unclear** — possibly a zombie old process (dual process scenario), or some other undiscovered code path. Jesse should check if multiple Archi processes are running (`tasklist | findstr python`). **Files:** `src/interfaces/discord_bot.py`.
...
- [ ] **Scheduled task payload blocked by garbage guard** — (Added 2026-03-06, session 213.) "Drink water" payload (11 chars, 2 words) was caught by `_is_garbage_notification` as garbage (`len < 15 and words <= 2`). Fixed: `_fire_scheduled_task` now wraps short payloads (<20 chars) with "Reminder: " prefix. **File:** `src/core/heartbeat.py`.

- [ ] **Scheduled tasks `times_fired: 0` — verify after next restart** — (Added 2026-03-06, session 213.) Both scheduled tasks ("remind-to-stretch" at 16:20, "drink-water" at 15:00) show `times_fired: 0` despite being past their next_run_at. Could be FUSE mount caching in Cowork session, or could be a real issue (croniter not installed on Windows, or advance_task failing). Jesse should verify after next restart: check if tasks fire at their scheduled times, and check `data/scheduled_tasks.json` for updated `times_fired`. **Files:** `src/core/scheduler.py`, `src/core/heartbeat.py`.
